...
@section('title')
  {{ __('PermissionsUI::permissions.permissions.title_edit_multi') }}
@endsection
  ...
  <div class="min-h-full flex flex-col justify-center sm:px-6 lg:px-8">
    <div class="sm:mx-auto sm:w-full sm:max-w-lg">
      <div class="px-6 py-12 bg-white shadow-sm sm:rounded-lg sm:px-12">
        <form action="{{ route('permission_ui.permissions.update_multi') }}" method="post">
          @csrf
          <div class="pb-12 border-b border-gray-900/10">
            ...
            <div class="space-y-6">
              ...
              <fieldset>
                <legend
                    class="font-semibold text-sm/6 text-gray-900">{{ __('PermissionsUI::permissions.permissions.title') }}</legend>
                <div class="mt-6 space-y-1">
                  @foreach($permissions as $permission)
                    <input id="permission-{{$permission->id}}" type="hidden" name="permissions[]"  value="{{ $permission->id }}"/>
                    <span class="inline-flex items-center rounded-md bg-gray-50 px-2 py-1 text-sm font-medium text-gray-600 ring-1 ring-gray-500/10 ring-inset">{{ $permission->name }}</span>
                  @endforeach
                  </div>
              </fieldset>
