...
@section('content')
  <div class="px-4 sm:px-6 lg:px-8">
    @include('PermissionsUI::shared.header', [
      'heading' => __('PermissionsUI::permissions.permissions.heading'),
      'link' => route(config('permission_ui.route_name_prefix') . 'permissions.create'),
        ...
        <div class="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8">
          <div class="shadow-sm ring-1 ring-black/5 sm:rounded-lg overflow-hidden">
            ...
            @include('PermissionsUI::shared.actions', [
              'editLink' => route(config('permission_ui.route_name_prefix') . 'permissions.edit_multi'),
              'deleteLink' => route(config('permission_ui.route_name_prefix') . 'permissions.delete_multi'),
              'arrayVar' => 'permissions[]',
            ])
              ...
              <tr>
                <th scope="col" class="relative px-7 sm:w-12 sm:px-6">
                  <div class="group absolute top-1/2 left-4 -mt-2 grid size-4 grid-cols-1">
                    <input type="checkbox"
                           onclick="toggleSelectAll({{count($permissions)}})"
                ...
                <th scope="col"
                    class="hidden px-3 py-3.5 font-semibold text-left text-sm text-gray-900 lg:table-cell">{{ __('PermissionsUI::permissions.permissions.fields.created_at') }}</th>
              ...
              @forelse($permissions as $permission)
                ...
                <tr id="tr-{{$formCount}}">
                  ...
                  <td class="relative py-4 pr-4 pl-3 flex justify-center items-center whitespace-nowrap sm:pr-6">
                    <a href="{{ route(config('permission_ui.route_name_prefix') . 'permissions.edit', $permission) }}"
                      ...
                      <span
                          class="sr-only">{{ __('PermissionsUI::permissions.global.edit') }} {{ $permission->name }}</span></a>
                    ...
                    <form id="form{{ $formCount }}" class="inline-block"
                      ...
                      <a href="#"
                        ...
                        <span
                            class="sr-only">{{ __('PermissionsUI::permissions.global.delete') }} {{ $permission->name }}</span></a>
                    </form>
                  </td>
                  <!-- End of Actions -->
                </tr>
