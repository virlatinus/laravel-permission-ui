...
@section('content')
    <form class="mt-2 mb-4" action="{{ route(config('permission_ui.route_name_prefix') . 'permissions.store') }}" method="post">
        @csrf
        ...
        @if($roles->count())
            <div class="mt-3">
                <label class="block xl:text-left" for="permissions">{{ __('PermissionsUI::permissions.roles.title') }}</label>
                <div class="ml-2">
                    @foreach($roles as $id => $name)
                        <div class="inline-flex ml-3 align-middle">
                            <input class="rounded-full border border-blue-600 mr-2 absolute mt-1 -ml-6" type="checkbox" name="roles[]" id="role-{{ $id }}" value="{{ $id }}" @checked(old('role-'. $id))>
