  ...
  <div class="min-h-full flex flex-col justify-center sm:px-6 lg:px-8">
    <div class="sm:mx-auto sm:w-full sm:max-w-lg">
      <div class="px-6 py-12 bg-white shadow-sm sm:rounded-lg sm:px-12">
        <form action="{{ route('permission_ui.users.update', $user) }}" method="post">
          @csrf
          @method('PATCH')
          <div class="pb-12 border-b border-gray-900/10">
            ...
            <div class="space-y-6">
              ...
              <div>
                ...
                <div class="mt-2">
                  <input id="name" type="text" name="name" required autocomplete="name" value="{{ old('name', $user->name) }}"
              ...
              <div>
                ...
                <div class="mt-2">
                  <input id="email" type="email" name="email" required autocomplete="email" placeholder="{{ __('PermissionsUI::permissions.users.fields.email') }}"
                         class="block w-full px-3 py-1.5 bg-white rounded-md outline-1 outline-gray-300 text-base text-gray-900 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6 -outline-offset-1 placeholder:text-gray-400" value="{{ old('email', $user->email) }}"/>
                </div>
                @error('email')
              ...
              @include('PermissionsUI::shared.password', [
                ...
                'label' => 'Password',
                'required' => false,
                'description' => __('PermissionsUI::permissions.global.ignore_password'),
              ])
                        ...
                        <div class="h-6 flex shrink-0 items-center">
                          <div class="grid grid-cols-1 group size-4">
                            <input type="checkbox" name="roles[]" id="role-{{ $id }}"
                                   ...
                                   @checked(in_array($id, old('roles', [])) || $user->roles->contains($id))
                                   value="{{ $id }}"
