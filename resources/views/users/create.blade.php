        ...
        <form action="{{ route(config('permission_ui.route_name_prefix') . 'users.store') }}" method="post">
          ...
          <div class="pb-12 border-b border-gray-900/10">
            ...
            <div class="space-y-6">
              ...
              <div>
                <label for="name"
                       class="block font-medium text-sm/6 text-gray-900">{{ __('PermissionsUI::permissions.users.fields.name') }}</label>
                <div class="mt-2">
                  <input id="name" type="text" name="name" required autocomplete="name"
                         placeholder="{{ __('PermissionsUI::permissions.users.fields.name') }}"
              ...
              <div>
                <label for="email" class="block font-medium text-sm/6 text-gray-900">{{ __('PermissionsUI::permissions.users.fields.email') }}</label>
                <div class="mt-2">
                  <input id="email" type="email" name="email" required autocomplete="email" placeholder="{{ __('PermissionsUI::permissions.users.fields.email') }}"
                         class="block w-full px-3 py-1.5 bg-white rounded-md outline-1 outline-gray-300 text-base text-gray-900 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6 -outline-offset-1 placeholder:text-gray-400"/>
                </div>
              </div>

              @include('PermissionsUI::shared.password', [
                'name' => 'password',
                'label' => 'Password',
              ])
              ...
              <div class="flex items-center justify-between">
                <span class="flex grow flex-col">
                  <label id="availability-label" class="text-sm/6 font-medium text-gray-900">{{ __('PermissionsUI::permissions.users.fields.verify.label') }}</label>
                  <span id="availability-description" class="text-sm text-gray-500">{{ __('PermissionsUI::permissions.users.fields.verify.description') }}</span>
                </span>
                <div
                    class="relative inline-flex w-10 h-5 shrink-0 justify-center items-center rounded-full outline-offset-2 outline-indigo-600 group has-focus-visible:outline-2 dark:outline-indigo-500">
                      <span
                          class="absolute inset-ring inset-ring-gray-900/5 w-9 h-4 mx-auto bg-gray-200 rounded-full transition-colors duration-200 ease-in-out group-has-checked:bg-indigo-600 dark:bg-gray-800/50 dark:inset-ring-white/10 dark:group-has-checked:bg-indigo-500"></span>
                  <span
                      class="absolute left-0 bg-white border border-gray-300 rounded-full shadow-xs transition-transform duration-200 ease-in-out size-5 group-has-checked:translate-x-5 dark:shadow-none"></span>
                  <input type="checkbox" name="verify" aria-label="{{ __('PermissionsUI::permissions.users.fields.verify.label') }}" checked
                         class="absolute inset-0 focus:outline-hidden appearance-none"/>
                </div>
              </div>
...
@push('child-scripts')
  @include('PermissionsUI::scripts.password-toggle')
@endpush