  ...
  <div class="min-h-full flex flex-col justify-center sm:px-6 lg:px-8">
    <div class="sm:mx-auto sm:w-full sm:max-w-lg">
      <div class="px-6 py-12 bg-white shadow-sm sm:rounded-lg sm:px-12">
        <form action="{{ route('permission_ui.roles.update', $role) }}" method="post">
          @csrf
          @method('PATCH')
          <div class="pb-12 border-b border-gray-900/10">
            ...
            <div class="space-y-6">
              ...
              <div>
                ...
                <div class="mt-2">
                  <input id="name" type="text" name="name" required autocomplete="name" value="{{ old('name', $role->name) }}"
              ...
              @if($permissions->count())
                <fieldset>
                  <legend
                      class="font-semibold text-sm/6 text-gray-900">{{ __('PermissionsUI::permissions.roles.fields.permissions') }}</legend>
                        ...
                        <div class="h-6 flex shrink-0 items-center">
                          <div class="grid grid-cols-1 group size-4">
                            <input type="checkbox" name="permissions[]" id="permission-{{ $id }}"
                                   ...
                                   @checked(in_array($id, old('permissions', [])) || $role->permissions->contains($id))
                                   value="{{ $id }}"
          ...
          <div class="mt-6 flex gap-x-6 justify-end items-center">
            <a class="rounded-sm bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-xs ring-1 ring-gray-300 ring-inset hover:bg-gray-50"
               href="{{ route(config('permission_ui.route_name_prefix').'roles.index') }}"
