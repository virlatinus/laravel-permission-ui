...
@section('content')
    <div class="px-4 sm:px-6 lg:px-8">
        @include('PermissionsUI::shared.header', [
          'heading' => __('PermissionsUI::permissions.roles.heading'),
          'link' => route(config('permission_ui.route_name_prefix') . 'roles.create'),
                ...
                <div class="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8">
                    <div class="shadow-sm ring-1 ring-black/5 sm:rounded-lg overflow-hidden">
                        ...
                        @include('PermissionsUI::shared.actions', [
                          'editLink' => route(config('permission_ui.route_name_prefix') . 'roles.edit_multi'),
                          'deleteLink' => route(config('permission_ui.route_name_prefix') . 'roles.delete_multi'),
                          'arrayVar' => 'roles[]',
                        ])
                            ...
                            <tr>
                                <th scope="col" class="relative px-7 sm:w-12 sm:px-6">
                                    <div class="group absolute top-1/2 left-4 -mt-2 grid size-4 grid-cols-1">
                                        <input type="checkbox"
                                               onclick="toggleSelectAll({{count($roles)}})"
                                ...
                                <th scope="col"
                                    class="py-3.5 pr-3 pl-4 font-semibold text-left text-sm text-gray-900 sm:pl-6 min-w-[170px]">{{ __('PermissionsUI::permissions.roles.fields.name') }}</th>
                                <th scope="col"
                                    class="px-3 py-3.5 font-semibold text-left text-sm text-gray-900">{{ __('PermissionsUI::permissions.roles.fields.permissions') }}</th>
                                <th scope="col"
                                    class="hidden px-3 py-3.5 font-semibold text-left text-sm text-gray-900 lg:table-cell">{{ __('PermissionsUI::permissions.roles.fields.created_at') }}</th>
                                    ...
                                    <td class="px-3 py-4 sm:flex-col lg:whitespace-nowrap">
                                        @foreach($role->permissions as $permission)
                                            @include('PermissionsUI::shared.badge', [
                                                'color' => $permissionColors[$permission->name],
                                                'name' => $permission->name,
                                                'link' => route(config('permission_ui.route_name_prefix') . 'roles.delete_permission', [$role, $permission]),
                                              ])
                                        @endforeach
                                    </td>
                                    <td
                                        class="tdl hidden px-3 py-5 text-sm whitespace-nowrap text-gray-500 lg:table-cell">{{ $role->created_at }}</td>

                                    <!-- Actions -->
                                    <td class="relative py-4 pr-4 pl-3 flex justify-center items-center whitespace-nowrap sm:pr-6">
                                        <a href="{{ route(config('permission_ui.route_name_prefix') . 'roles.edit', $role) }}"
                                           ...
                                           role="button">
                                            ...
                                            <span class="sr-only">{{ __('PermissionsUI::permissions.global.edit') }} {{ $role->name }}</span></a>

                                        <form id="form{{ $formCount }}" class="inline-block"
                                              action="{{ route(config('permission_ui.route_name_prefix') . 'roles.destroy', $role) }}"
                                              method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <a href="#"
                                               ...
                                               role="button">
                                                ...
                                                <span
                                                    class="sr-only">{{ __('PermissionsUI::permissions.global.delete') }} {{ $role->name }}</span></a>
                                        </form>
                                    </td>
                                    <!-- End of Actions -->
                                </tr>
