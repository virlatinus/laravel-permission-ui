...
@section('content')
    <div class="mt-4 mb-4 flex">
        <a class="inline-block align-middle text-center select-none border font-normal whitespace-nowrap rounded-sm py-1 px-3 leading-none no-underline {{ config('permission_ui.save_button_classes') }} px-3 py-2" href="{{ route(config('permission_ui.route_name_prefix') . 'roles.create') }}" role="button">{{ __('PermissionsUI::permissions.global.create') }}</a>
    </div>
                    ...
                    <tr>
                        ...
                        <td>
                            @foreach($role->permissions as $permission)
                                <span class="rounded-full py-2 px-4 bg-blue-600 px-2 py-1 text-xs text-white">{{ __($permission->name) }}</span>
                            @endforeach
                        </td>
                        <td>{{ $role->created_at }}</td>
                        <td class="px-3" style="border-left-width: 2px">
                            <a class="inline-block align-middle text-center select-none border font-normal whitespace-nowrap rounded-sm py-1 px-3 leading-none no-underline {{ config('permission_ui.edit_button_classes') }} px-3 py-2" href="{{ route(config('permission_ui.route_name_prefix') . 'roles.edit', $role) }}" role="button">
